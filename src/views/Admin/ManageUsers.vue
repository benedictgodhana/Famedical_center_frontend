<template>
    ...
    <v-alert v-model="addSuccessSnackbarVisible" color="success">
      ...
      <v-card variant="outlined" style="width: 100%; margin-top: 20px;">
        ...
        <v-card-title style="background:purple;color:white">User's Table</v-card-title>
        <v-card-text style="margin-top: 10px;">
          ...
          <v-btn @click="openAddUserDialog" color="purple" style="border-radius: 5px;text-transform: capitalize;"><span class="mdi mdi-account-plus-outline"></span>Add User</v-btn>
        </v-card-text>
        <v-card-text style="margin-top: 0px;">
          ...
          >
            <template v-slot:item.action="{ item }">
              <v-btn color="transparent" elevation="0" class="mr-4" @click="openEditDialog(item)" style="text-transform:capitalize;"><v-icon color="success">mdi-pen</v-icon></v-btn>
              <v-btn color="transparent" elevation="0" @click="deleteUserDialog(item)"  style="text-transform:capitalize;"><v-icon color="red">mdi-delete</v-icon></v-btn>
              <v-btn elevation="0" style="text-transform: capitalize;" class="mr-4" @click="viewUSerDialog(item)"><v-icon>mdi-eye</v-icon></v-btn>
...
></v-select>
  ...
  try {
    ...
    let requestData = {
      ...
      ...(this.newUser.role === 'doctor' ? { service_id: this.newUser.service_id   } : {})
