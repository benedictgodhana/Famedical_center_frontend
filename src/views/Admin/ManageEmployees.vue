<template>
  <v-container>
    ...
    <v-card variant="outlined" style="width: 100%; margin-top: 20px;" v-if="employees.length > 0">
      <v-card-title style="background:purple;color:white">Doctors with Services</v-card-title>
      ...
      <v-card-text style="margin-top: 20px;">
        ...
        >
          <template v-slot:item.action="{ item }">
            <!-- Action buttons -->
            <v-btn color="" elevation="0" style="text-transform: capitalize;" @click="editEmployeeDialog(item)"><v-icon color="success">mdi-pen</v-icon></v-btn>
            <v-btn elevation="0" style="text-transform: capitalize;" class="mr-4" @click="viewEmployeeDialog(item)"><v-icon>mdi-eye</v-icon></v-btn>
          </template>
        </v-data-table>
      </v-card-text>
    </v-card>
...
export default {
  ...
  data() {
    return {
      ...
      tableHeaders: [
        { title: 'Doctor Name', value: 'user_name' },
        { title: 'Doctor Email', value: 'user_email' },
        { title: 'Service Offered', value: 'service_name' },
  ...
  methods: {
    searchEmployees() {
      this.filteredEmployees = this.employees.filter(employee =>
        employee.user_name.toLowerCase().includes(this.searchQuery.toLowerCase())
      );
    },
